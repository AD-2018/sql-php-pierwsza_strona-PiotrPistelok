...
echo('<table border="1">');
    ...
    $sql ="select imie,dzial,MAX(YEAR(CURDATE()) - YEAR(data_urodzenia)) as wiek from pracownicy,organizacja where id_org=dzial and imie not like '%a' group by imie,dzial order by wiek desc limit 1"; 
  echo($sql);
$result = mysqli_query($conn, $sql);
if ( $result) {
        echo "<li>ok";
